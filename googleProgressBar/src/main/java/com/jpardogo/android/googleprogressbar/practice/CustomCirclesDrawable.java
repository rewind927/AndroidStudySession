			...
			for (int i = 0; i < (360 / SWEEP_ANGLE); i++) {
				switch (i % 4) {
					case 0:
						...
						break;
					case 1:
						canvas.drawArc(mainOval, startAngel + i * SWEEP_ANGLE, SWEEP_ANGLE, true, bluePaint);
						break;
					case 2:
						canvas.drawArc(mainOval, startAngel + i * SWEEP_ANGLE, SWEEP_ANGLE, true, yellowPaint);
						break;
					case 3:
						canvas.drawArc(mainOval, startAngel + i * SWEEP_ANGLE, SWEEP_ANGLE, true, greenPaint);
						break;
				}
			}
			path.reset();
			path.moveTo(diameter / 2, diameter / 2);
			path.arcTo(mainOval, startAngel, startAngel + SWEEP_ANGLE);
