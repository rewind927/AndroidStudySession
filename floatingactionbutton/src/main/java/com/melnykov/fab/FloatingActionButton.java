...
public class FloatingActionButton extends ImageButton {
    ...
    private void toggleTop(final boolean visible, final boolean animate, boolean force) {
        if (mVisible != visible || force) {
            mVisible = visible;
            int height = getHeight();
            if (height == 0 && !force) {
    ...
    private void toggleScaleAnimation(final boolean visible, final boolean animate, boolean force) {
        if (mVisible != visible || force) {
            mVisible = visible;
            int height = getHeight();
            if (height == 0 && !force) {
    ...
    private class RecyclerViewScrollDetectorImpl extends RecyclerViewScrollDetector {
        ...
        @Override
        public void onScrollDown() {
            toggleScaleAnimation(true, true, false);
            if (mScrollDirectionListener != null) {
                mScrollDirectionListener.onScrollDown();
            }
        }
        ...
        @Override
        public void onScrollUp() {
            toggleScaleAnimation(false, true, false);
            if (mScrollDirectionListener != null) {
                mScrollDirectionListener.onScrollUp();
            }
        }
